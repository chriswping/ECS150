 ...
 */
  ...
  if ( (dst_ptr->p_rts_flags & (RECEIVING | SENDING)) == RECEIVING &&
	...
	++(caller_ptr->nummes[dst_ptr->p_nr + NR_TASKS]);
 ...
 */
  ...
  /* If the current process has the lower CPU time compared to every other 
   * process in its queue, put it in the front. Otherwise, put them in the back 
   * of the queue
   */
  for( xpp = &rdy_head[q]; *xpp != NIL_PROC; xpp =&(*xpp) -> p_nextready){
    
    if((rp -> p_user_time + rp -> p_sys_time) >
      ((*xpp) -> p_user_time + (*xpp) -> p_sys_time)){
      *front = 0;  
      break;
    }
    else{
