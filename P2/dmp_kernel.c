...
PUBLIC void kmessages_dmp()
{
    ...
    static struct proc *header = BEG_PROC_ADDR;
/* caller process & pointers to print all rows */

    int i = 0, j = 0, m_rows = 0;
/* column, row and max row counters */
    
    if(sys_getproctab(proc) != OK)
        return;
    
    printf("Messages sent from process i to process j\n");
    printf("%11s");
    

    for(rp = header; j < 12; ++rp){
        if(isemptyp(rp))
            continue;
        ++j;
        if(rp->p_nr == -2 || rp ->p_nr == -1 || rp->p_nr == 3 || rp ->p_nr == 6)
            printf("%7s", rp->p_name);
        else if(rp->p_nr == -4 || rp->p_nr == 1 || rp->p_nr == 0 || rp->p_nr == 7)
            printf("%5s", rp->p_name);
        else if(rp->p_nr == 2)
            printf("%3s", rp->p_name);
        else if(rp->p_nr == 4 || rp->p_nr == 5)
            printf("%4s", rp->p_name);
        else
            printf("%6s", rp->p_name);
    }
    ...
    for(rp = oldrp; rp < END_PROC_ADDR; ++rp){
        ...
        for(i = 0; j < 12; ++i){
            ...
            ++j;
            if(j == 2 || j == 3 || j == 7 || j == 10)
                printf("%7d", rp->nummes[i]);
            else if(j == 0 || j == 4 || j == 5 || j == 11)
                printf("%5d", rp->nummes[j]);
            else if(j == 6)
                printf("%3d", rp->nummes[j]);
            else if(j == 8 || j == 9)
                printf("%4d", rp->nummes[j]);
            else
                printf("%6d", rp->nummes[j]);
        }
        /* prints number of messages sent */
        printf("\n");
    }
    if(rp == END_PROC_ADDR)
        rp = BEG_PROC_ADDR;
        ...
        printf("--more--\r");
    oldrp = rp;
  /*save the current ptr position for next iteration*/
}
