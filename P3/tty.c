...
PRIVATE struct termios termios_defaults = {
  TINPUT_DEF, TOUTPUT_DEF, TCTRL_DEF, TLOCAL_DEF, TSPEED_DEF, TSPEED_DEF,
  {
	...
	TREPRINT_DEF, TLNEXT_DEF, TDISCARD_DEF, TDELN_DEF, TPASTE_DEF,
  },
};
 ...
 */
  ...
  /* Detect ^K */
  if(flag == 0 && ch == tp->tty_termios.c_cc[VDELN]){
    flag = 1;
    continue;
	}
  ...
  if(flag == 1){
    bnum = (char) ch;
    bnum = bnum - '0';
    if(bnum > tp->tty_incount)
        bnum = tp->tty_incount;
		dnum = bnum;
	    for(i = 0; i < dnum; i++){
          int k = tp -> tty_incount - 1;
          (tp -> savebuf)[i] = (tp->tty_intail)[k];
          (void) back_over(tp);
		}
     flag = 0;
     continue;
    ...
    if( ch == tp->tty_termios.c_cc[VPASTE]){
		...
		for( i = dnum - 1; i > -1; i--){
		  ...
		  tp->tty_incount++;	
	  	  (tp->tty_inbuf)[tp->tty_incount - 1] = (tp -> savebuf)[i];
		  tp->tty_inhead++;
		  ch = (tp -> savebuf)[i];
          (void)tty_echo(tp, ch);
	      }
