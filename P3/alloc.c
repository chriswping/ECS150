...
PUBLIC void printHole(void){
	struct hole *ptr = hole_head; int i = 0;
	while(ptr != 0 ){
		printf("%d: base: %d	len: %d\n",i, ptr->h_base, ptr->h_len);
		ptr = ptr->h_next;	
		i++;
	}
}
 ...
 */
  register struct hole *hp, *prev_ptr, *max_hole, *max_prev;
  phys_clicks old_base;
	phys_clicks max_len = 0;
	int result;
